...
void makeObjTorus(object3D_t &obj) {
	...
	for (int i = 0; i < n; i++)
	{
		...
		for (int j = 0; j < n; j++)
		{
			...
			col[(n*i) + j].r = rand() % 256;
			col[(n*i) + j].g = rand() % 256;
			col[(n*i) + j].b = rand() % 256;
...
void drawThorus()
{
	...
	// Save to File
	writeObjectToFile("thorus.off", obj);
